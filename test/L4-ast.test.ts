import { isBoolExp, isNumExp, isPrimOp, isStrExp, isVarDecl, isVarRef, isSetExp,
    ...
    parseL4, unparse, parseL4Exp, Exp, isTraceExp } from '../src/L4/L4-ast';
...
describe("HW3 trace expression", () => {
    it('HW3 trace expressions', () => {
        const traceExp1 = p("(trace foo)");
        expect(traceExp1).toSatisfy(isOkT(isTraceExp));
        if (isOkT(isTraceExp)(traceExp1)) {
            expect(traceExp1.value.var).toSatisfy(isVarRef);
        }
    })
})
